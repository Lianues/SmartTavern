...
export function initCatalogBridge(bus: EventBus): void {
  ...
  bus.on(CatalogChannel.EVT_CATALOG_LLMCONFIGS_REQ, async (payload: any) => {
    ...
    try {
      ...
      CatalogChannel.llmConfigs.value = items
  ...
  bus.on(CatalogChannel.EVT_CATALOG_CHARACTER_UPDATE_REQ, async (payload: any) => {
    const { file, content, name, description, tag } = payload || {}
    
    try {
      const res = await DataCatalog.updateCharacterFile(file, content, name, description)
  ...
  bus.on(CatalogChannel.EVT_CATALOG_PERSONA_UPDATE_REQ, async (payload: any) => {
    ...
    try {
      const res = await DataCatalog.updatePersonaFile(file, content, name, description)
  ...
  bus.on(CatalogChannel.EVT_CATALOG_PRESET_UPDATE_REQ, async (payload: any) => {
    ...
    try {
      const res = await DataCatalog.updatePresetFile(file, content, name, description)
  ...
  bus.on(CatalogChannel.EVT_CATALOG_WORLDBOOK_UPDATE_REQ, async (payload: any) => {
    ...
    try {
      const res = await DataCatalog.updateWorldBookFile(file, content, name, description)
  ...
  bus.on(CatalogChannel.EVT_CATALOG_REGEX_UPDATE_REQ, async (payload: any) => {
    ...
    try {
      const res = await DataCatalog.updateRegexRuleFile(file, content, name, description)
  ...
  bus.on(CatalogChannel.EVT_CATALOG_LLMCONFIG_UPDATE_REQ, async (payload: any) => {
    ...
    try {
      const res = await DataCatalog.updateLLMConfigFile(file, content, name, description)

...
function attachCompletionHandlersForTag(tagNodeId) {
  ...
  const offSaved = Host.events.on(Completion.EVT_COMPLETION_SAVED, ({ tag, node_id, doc, content }) => {
    if (tag !== tagNodeId) return
    if (hasReceivedError) return
 ...
 */
const offAssistPlaceholder = Host.events.on(Threaded.EVT_THREAD_ASSIST_PLACEHOLDER_CREATE, ({ conversationFile, tempNodeId }) => {
  if (conversationFile !== props.conversationFile) return
  // 创建助手占位消息
  props.messages.push({ id: tempNodeId, role: 'assistant', content: '' })
...
async function regenerateMessage(msg) {
  ...
  try {
    if (msg.role === 'assistant') {
      ...
      const offOk = Host.events.on(Branch.EVT_BRANCH_RETRY_ASSIST_OK, ({ conversationFile, newNodeId, doc, latest, active_path, tag: rtag }) => {
        if (conversationFile !== props.conversationFile || rtag !== tag) return

        // 替换列表中的该消息为新分支节点，并设置等待态（占位由工作流统一）
        const idx = props.messages.findIndex(m => m.id === oldId)
        if (idx >= 0) {
          props.messages[idx] = { id: newNodeId, role: 'assistant', content: '' }
          ensurePaletteFor(props.messages[idx])
      ...
      const offOk = Host.events.on(Branch.EVT_BRANCH_RETRY_USER_OK, ({ conversationFile, action, assistantNodeId, doc, tag: rtag }) => {
        ...
        if (action === 'retry_assistant' && assistantNodeId) {
          ...
          // 监听工作流触发的助手分支创建事件
          const offAssistOk = Host.events.on(Branch.EVT_BRANCH_RETRY_ASSIST_OK, ({ conversationFile: cf, newNodeId, doc: newDoc, latest: latest2, active_path: ap2, tag: assTag }) => {
            // 检查是否是我们触发的重试（通过检查时间戳）
            if (cf !== props.conversationFile) return
            if (!assTag || !assTag.startsWith('retry_user_assist_')) return
            
            // 找到旧的助手消息并替换为新分支节点
            const idx = props.messages.findIndex(m => m.id === oldAssistantId)
            if (idx >= 0) {
              props.messages[idx] = { id: newNodeId, role: 'assistant', content: '' }
              ensurePaletteFor(props.messages[idx])
              nodeStates.value[newNodeId] = { waitingAI: true, waitingSeconds: 0, error: null }
